...
export default function Pagination({ currentPage, totalPages, onPageChange, totalCount, pageSize }: Props) {
  if (totalPages <= 1) {
    return (
  ...
  return (
    <div className="flex flex-col items-center gap-3 mt-6">
      <div className="flex items-center gap-1">
        ...
        {pages.map((p, i) =>
          p === '...' ? (
            <span key={`e-${i}`} className="px-2 text-[#64748B] text-sm">…</span>
          ) : (
            <button key={p} type="button" onClick={() => onPageChange(p as number)}
              className={`min-w-[36px] px-2 py-1.5 text-sm rounded-md border transition ${
                p === currentPage ? 'bg-[#6B31F5] text-white border-[#6B31F5]' : 'border-[#E2E8F0] text-[#0F172A] hover:bg-gray-50'
              }`}>
              {p}
            </button>
          ),
      ...
      <div className="text-sm text-[#64748B]">Showing {start}–{end} of {totalCount} jobs</div>
    </div>
  );
}
